### Download and install NixOS
  ...
  Plug your target machine to the power source and to your network switch using an RJ45 cable.

  Plug-in the screen, the keyboard and the mouse (use a wired mouse to avoid issues, some wireless ones don't work so great and the pointer may jerk around on the screen). These are **used only during this first guided installation procedure**, after this all interactions with the target machine will be done "headless" over ssh.

  

  Stick the USB stick in your target machine and switch it on, keep pressing the ``<Delete>`` or ``<ESC>`` key on the keyboard during boot to access the BIOS settings.
...
-  ``Boot Option #1`` set to ``USB Device: [USB]``
  ...
  ![](../docs/img/NixOS_install_screenshots/NixOS_install_screenshot_003.png)

  Let the machine boot from the USB stick.
  ...
  Now we are going to configure the storage devices and partitions. Our target machine has 2 built-in storage devices:
  ...
  - ```/dev/sdb``` SATA 1TB SSD used for *states*: This is where the system settings, the bitcoin blockchain and installed software settings as well as user data is stored. The data on this drive is *persisted*.
  ...
  The installation takes less than one minute click on "Toggle Logs" at the bottom right of the splash screen to see and understand how the OS is being pulled and installed.
  ...
  Remove the USB stick off the target machine before it restarts so it will boot on the MBR of /dev/sda. Your system is now running by itself, let's continue its configuration.

  <div id="configuration.nix" ></div>
  Congratulations, after reboot your new NixOS system is installed, you can login on the console using user ```root``` password ```mybonk```.
