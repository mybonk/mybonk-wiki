...
cmd_update() {
    ...
    if [ -z "$name" ]; then
        ...
        echo "Usage: $0 update <container-name>"
        exit 1
    fi
    ...
    if [ -f "$conf_file" ]; then
        ...
        # - LOCAL_ADDRESS must be empty (for DHCP)
        # - HOST_ADDRESS must be empty (for DHCP)
        local needs_fix=false

        if ! grep -q "^HOST_BRIDGE=br-containers$" "$conf_file"; then
            needs_fix=true
        fi

        if ! grep -q "^LOCAL_ADDRESS=$" "$conf_file"; then
            needs_fix=true
        fi

        if ! grep -q "^HOST_ADDRESS=$" "$conf_file"; then
            needs_fix=true
        fi

        if [ "$needs_fix" = true ]; then
            echo "⚠ Bridge configuration needs fixing (requires container restart)"
            needs_restart=true
            ...
            if grep -q "^HOST_BRIDGE=" "$conf_file"; then
                sed -i 's/^HOST_BRIDGE=.*/HOST_BRIDGE=br-containers/' "$conf_file"
            else
                echo "HOST_BRIDGE=br-containers" >> "$conf_file"
            fi

            # Fix or add LOCAL_ADDRESS (set to empty for DHCP)
            if grep -q "^LOCAL_ADDRESS=" "$conf_file"; then
                sed -i 's/^LOCAL_ADDRESS=.*/LOCAL_ADDRESS=/' "$conf_file"
            else
                echo "LOCAL_ADDRESS=" >> "$conf_file"
            fi

            # Fix or add HOST_ADDRESS (set to empty for DHCP)
            if grep -q "^HOST_ADDRESS=" "$conf_file"; then
                sed -i 's/^HOST_ADDRESS=.*/HOST_ADDRESS=/' "$conf_file"
            else
...
COMMANDS:
  ...
  update <name>           Update container configuration from flake
                          - Stops container, updates config, restarts
                          - Preserves all data (blockchain, secrets, etc.)
                          - Auto-fixes bridge configuration if needed
